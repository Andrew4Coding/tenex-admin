...
export async function ModelDetailLoader({ request, params }: LoaderFunctionArgs) {
  ...
  if (!(model in prisma) || typeof (prisma as any)[model]?.findUnique !== 'function') {
  ...
  const modelFields = modelMetadata.fields.filter((field) => {
  ...
  // Find related models
  let relatedModels = Object.keys(prismaMetaData.models).filter((m) => {
    return prismaMetaData.models[m].fields.some((f) => f.type === model && f.kind === 'object');
  });
  ...
  return { item, modelName: model, modelFields, relatedModels: finalRelatedModels };
}
